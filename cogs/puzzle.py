...
class MindMeldButton(discord.ui.Button["MindMeld"]):
    ...
    def __init__(self, x: int, y: int, label: str, disabled: bool, character: Character) -> None:
        super().__init__(
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        embed = discord.Embed(title="Mind Meld!", description=content, color=COLOR)
...
class MindMeld(discord.ui.View):
    ...
    def __init__(
        ...
        self.board = [[temp.pop(0) for _ in range(level)] for _ in range(level)]

        self.game_level = 1

        for i in range(level):
            for j in range(level):
                self.add_item(MindMeldButton(i, j, self.board[j][i], self.disabled, self.character))
...
class Puzzle(commands.Cog):
    ...
    async def anagram_adventure(self, interaction: discord.Interaction) -> None:
        character = character_db.get_character_info(interaction.user.id)
        ...
        correct_guess = 0
        end_time = datetime.datetime.now() + datetime.timedelta(seconds=120)  # noqa: DTZ005
        while datetime.datetime.now() < end_time:  # noqa: DTZ005
            try:
    ...
    @app_commands.describe(level="Select a level", start_time="Select a start time")
    ...
    async def mind_meld(self, interaction: discord.Interaction, level: int, start_time: int) -> None:
        character = character_db.get_character_info(interaction.user.id)
        ...
        words = [temp.pop(random.randint(0, len(temp) - 1)) for _ in range(level**2)]
        ...
        embed = discord.Embed(
            title="Mind Meld!",
            description=f"Find `{' '.join(winning_words)}`, Hurry up! You have `{start_time}` seconds remember the positions!",  # noqa: E501
            color=COLOR,
        )
        await interaction.response.send_message(view=view, delete_after=start_time, embed=embed)
