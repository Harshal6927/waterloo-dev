...
class DropdownView(discord.ui.View):
    def __init__(self) -> None:
        ...
        sex_options: list[discord.SelectOption] = [
            discord.SelectOption(label="Male", description="Start your life as a male."),
            discord.SelectOption(label="Female", description="Start your life as a female."),
            discord.SelectOption(label="Other", description="Start your life as another gender."),
        ]
        ...
        self.add_item(Dropdown(placeholder="Select sex", options=sex_options, custom_id="sex"))
        self.add_item(
            Dropdown(
...
class SubmitButton(discord.ui.Button):
    def __init__(self) -> None:
        ...
        super().__init__(label="Submit", style=discord.ButtonStyle.green, custom_id="submit")
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        if sex and race and clss:
            ...
            if char_info != -1:
                await interaction.response.send_message("You already have a character created.", ephemeral=True)
            else:
...
class CharacterHandle(commands.Cog):
    ...
    async def view(self, interaction: discord.Interaction) -> None:
        ...
        if character == -1:
            await interaction.response.send_message("You do not have a character created.", ephemeral=True)
            return

        formatted_quest_log = character.get_pretty_quest_log()
        formatted_equipment_list = character.get_pretty_equipment_list()

        formatted_quest_log = formatted_quest_log if formatted_quest_log else "No quests yet"
        ...
        if character != -1:
            ...
            embed = discord.Embed(
                ...
                description=f"**Quest Log:**\n{formatted_quest_log}\n\n**Sex:** {sex}\n**Race:** {race}\n**Class:** {class_}\n\n**Equipment:**\n{formatted_equipment_list}",  # noqa: E501
                color=0x00FF00,
            )
    ...
    async def balance(self, interaction: discord.Interaction) -> None:
        ...
        if character == -1:
            ...
            await interaction.response.send_message(f"Your balance is `{character.coins}` coins.")
    ...
    async def checkin(self, interaction: discord.Interaction) -> None:
        ...
        if check < 0:
            ...
            await interaction.response.send_message("You have checked in your character, use `/view` to see changes")

    @app_commands.command(name="gacha", description="Roll the gacha to get a random item")
    async def gacha(self, interaction: discord.Interaction) -> None:
        ...
        await interaction.response.send_message(f"You have rolled the gacha and received a `{generated_item.name}`.")

    @app_commands.command(name="leaderboard", description="Show ranking points leaderboard")
    async def leaderboard(self, interaction: discord.Interaction) -> None:
        top20 = character_db.get_character_leaderboard()
        top20 = [f"`{place}. {character.name} ({character.coins} coins)`" for place, character in enumerate(top20)]
        if top20:
            await interaction.response.send_message("\n".join(top20))
        else:
            await interaction.response.send_message("You have checked in your character, use `/view` to see changes")
